    ...
    *,
    ...
    return Video(
        ...
        channel=VideoChannel(
            id="channel-1",
            name="Chess Channel",
            url=("https://www.youtube.com/@chess-channel"),
            subscribers=50000,
        ),
        language="fr",
    )
...
def build_collection(
    *,
    recommendations: list[VideoRecommendation] | None = None,
) -> VideoCollection:
...
def build_wikichess_context(
    *,
    title: str = "Ruy Lopez",
    wikichess_title: str | None = ("Ruy Lopez Opening"),
    eco: str | None = "C60",
) -> RetrievalContext:
...
def test_get_youtube_service_returns_configured_service(
    ...
    monkeypatch.setattr(
        "app.agent.nodes.F_retrieve_videos.get_configured_service",
        configured_service,
    )
    ...
    result = _get_youtube_service(config)
...
def test_get_wikichess_identity_without_context(
    ...
    assert _get_wikichess_identity(state) == (
        None,
        None,
    )
...
def test_get_wikichess_identity_with_empty_context(
    ...
    assert _get_wikichess_identity(current_state) == (
        None,
        None,
    )
...
def test_build_lichess_search_context(
    ...
    context = _build_lichess_search_context(opening_state)
...
def test_build_lichess_search_context_without_opening(
    ...
    assert _build_lichess_search_context(state) is None
...
def test_build_wikichess_search_context(
    ...
    current_state = state.model_copy(
        update={
            "retrieval_context": (build_wikichess_context()),
        }
    )

    context = _build_wikichess_search_context(current_state)

    assert context is not None

    assert context.query == ("Ruy Lopez Opening C60")

    assert context.title == "Ruy Lopez Opening"

    assert context.eco == "C60"

    assert context.source == SEARCH_SOURCE_WIKICHESS
...
def test_build_wikichess_search_context_uses_opening_eco(
    ...
    assert context.query == "Ruy Lopez Wikichess C60"
...
def test_build_wikichess_search_context_without_title(
    ...
    assert _build_wikichess_search_context(current_state) is None
...
def test_build_search_context_prefers_lichess(
    ...
    current_state = opening_state.model_copy(
        update={
            "retrieval_context": (
                build_wikichess_context(
                    wikichess_title=("Titre Wikichess"),
                    eco="C61",
                )
            ),
        }
    ...
    context = _build_search_context(current_state)

    assert context is not None

    assert context.source == SEARCH_SOURCE_LICHESS
...
def test_build_search_context_returns_none(
    ...
    assert _build_search_context(state) is None
...
def test_build_search_request() -> None:
    ...
    request = _build_search_request(context)
    ...
    assert request.query == "Ruy Lopez C60"

    assert request.max_results == settings.youtube_search_max_results

    assert request.language == settings.youtube_default_language
...
def test_select_videos() -> None:
    ...
    recommendations = [
        ...
        for index in range(settings.max_selected_videos + 2)
    ]
    ...
    assert len(videos) == settings.max_selected_videos

    assert all(isinstance(video, Video) for video in videos)

    assert videos[0].id == "video-0"
...
def test_select_videos_returns_empty_list() -> None:
    ...
    assert _select_videos(collection) == []
...
def test_build_video_summary(
    ...
    assert "- Titre : Ruy Lopez Chess Opening" in summary

    assert "- Chaîne : Chess Channel" in summary

    assert f"- URL : {VIDEO_URL}" in summary

    assert "- Miniature :" in summary

    assert "- Publication : 2026-08-18T10:00:00Z" in summary

    assert "- Langue : fr" in summary

    assert "- Durée : 630 secondes" in summary
...
def test_build_video_summary_without_optional_values() -> None:
    ...
    video = Video(
        id="video-minimal",
        title="Chess Opening",
        url=("https://www.youtube.com/watch?v=video-minimal"),
        channel=VideoChannel(
            name="Chess Channel",
        ),
    )
...
def test_build_videos_summary(
    ...
    assert "Ruy Lopez Chess Opening" in summary

    assert "Italian Game Guide" in summary


def test_build_videos_summary_returns_none() -> None:
    """Vérifie l'absence de vidéos."""

    assert _build_videos_summary([]) is None
...
def test_build_workflow_context(
    ...
    assert context.videos_summary is not None

    assert "Ruy Lopez Chess Opening" in context.videos_summary

    assert state.workflow_context.videos_summary is None
...
def test_build_workflow_context_without_videos(
    ...
    assert context.videos_summary is None
...
def test_build_completed_update(
    ...
    assert update["current_step"] == WorkflowStep.RETRIEVE_VIDEOS

    assert WorkflowStep.RETRIEVE_VIDEOS in update["completed_steps"]

    assert update["videos"] == [
        video,
    ]

    assert update["workflow_context"].videos_summary is not None
...
def test_build_warning_update(
    ...
    assert update["workflow_context"].videos_summary is None
...
def test_build_error_update(
    ...
    assert WorkflowStep.RETRIEVE_VIDEOS not in update["completed_steps"]
...
def test_build_missing_service_update(
    ...
    assert update["errors"][0].code == ERROR_CONFIGURATION
...
def test_build_youtube_warning_update(
    ...
    assert update["warnings"][0].code == ERROR_YOUTUBE_UNAVAILABLE

    assert update["warnings"][0].message == str(error)
...
def test_build_unexpected_error_update(
    ...
    update = _build_unexpected_error_update(state)
...
async def test_retrieve_videos_missing_service(
    ...
    monkeypatch.setattr(
        "app.agent.nodes.F_retrieve_videos._get_youtube_service",
        get_service,
    )

    monkeypatch.setattr(
        "app.agent.nodes.F_retrieve_videos.emit_progress",
        emit_progress,
    )
...
async def test_retrieve_videos_success(
    ...
    assert len(update["videos"]) == 1

    assert update["videos"][0].id == VIDEO_ID

    search_videos.assert_awaited_once()

    request = search_videos.call_args.args[0]
...
async def test_retrieve_videos_success_with_wikichess_fallback(
    ...
    current_state = state.model_copy(
        update={
            "retrieval_context": (
                build_wikichess_context(
                    wikichess_title=("Italian Game Wikichess"),
                    eco="C50",
                )
            ),
        }
    ...
    assert request.query == "Italian Game Wikichess C50"
