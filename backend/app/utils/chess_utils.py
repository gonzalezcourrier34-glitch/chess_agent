...
def normalize_fen(
    ...
    if not isinstance(fen, str):
        raise InvalidFenError(
            message=(
                "La position FEN doit être une chaîne "
                "de caractères."
            )
        )
    ...
    if not normalized_fen:
        raise InvalidFenError(
            message=(
                "La position FEN ne peut pas être vide."
            )
        )
...
@overload
def normalize_notation(
    notation: MoveNotation | str | None,
    *,
    ...
    *,
    ...
    if notation is None:
        if required:
            raise InvalidNotationError(
                message=(
                    "La notation du coup est obligatoire."
                )
            )
    ...
    if isinstance(notation, MoveNotation):
        if notation not in SUPPORTED_NOTATIONS:
            raise InvalidNotationError(
                message=(
                    "La notation fournie n'est pas prise "
                    "en charge."
                )
            )
    ...
    if not isinstance(notation, str):
        raise InvalidNotationError(
            message=(
                "La notation du coup doit être une chaîne."
            )
        )
    ...
    if not normalized_notation:
        raise InvalidNotationError(
            message=(
                "La notation du coup ne peut pas être vide."
            )
        )

    try:
        parsed_notation = MoveNotation(
            normalized_notation
        )
        ...
        raise InvalidNotationError(
            message=(
                "La notation doit être 'uci' ou 'san'."
            )
        ) from error
...
def validate_board(
    ...
    if not isinstance(board, chess.Board):
        raise InvalidBoardStateError(
            message=(
                "L'échiquier doit être une instance de "
                "chess.Board."
            )
        )
    ...
    if not board.is_valid():
        raise InvalidBoardStateError(
            message=(
                "La position d'échecs est invalide."
            )
        )
...
def validate_chess_move(
    ...
    if not isinstance(move, chess.Move):
        raise InvalidMoveError(
            message=(
                "Le coup doit être une instance de "
                "chess.Move."
            )
        )
    ...
    if move == chess.Move.null():
        raise InvalidMoveError(
            message=(
                "Le coup nul n'est pas autorisé."
            )
        )
