...
class DocumentNextMove(BaseModel):
    ...
    move: str = Field(
        ...,
        min_length=1,
        description=("Coup permettant d'accéder à une continuation documentaire."),
    )

    source_url: str = Field(
        ...,
        min_length=1,
        description=("URL du document correspondant à la continuation."),
    )
...
class DocumentMetadata(BaseModel):
    ...
    source: str = Field(..., min_length=1, description="Source documentaire.")

    language: str = Field(default="en", min_length=2, description="Langue du document.")

    author: str | None = Field(default=None, description="Auteur du document.")

    url: str | None = Field(default=None, description="URL du document source.")

    publication_date: str | None = Field(
        default=None, description="Date de publication du document."
    )

    # Wikichess

    eco: str | None = Field(
        default=None, description="Code ECO associé au document Wikichess."
    )

    moves: tuple[str, ...] = Field(
        default_factory=tuple,
        description=("Séquence de coups conduisant au document Wikichess."),
    )
    ...
    position_after: str | None = Field(
        default=None,
        description=(
            "Dernier coup indiqué par le marqueur Position after de Wikichess."
        ),
    )

    wikichess_title: str | None = Field(
        default=None, description="Titre original fourni par Wikichess."
    )

    next_moves: tuple[DocumentNextMove, ...] = Field(
        default_factory=tuple,
        description=(
            "Continuations Wikichess disponibles depuis la position documentaire."
        ),
    )
...
class Document(BaseModel):
    ...
    id: str = Field(..., min_length=1, description="Identifiant unique du document.")

    type: DocumentType = Field(..., description="Nature du document.")

    title: str = Field(..., min_length=1, description="Titre du document.")

    content: str = Field(..., description="Contenu intégral du document.")
...
class DocumentChunk(BaseModel):
    ...
    id: str = Field(..., min_length=1, description="Identifiant unique du fragment.")

    document_id: str = Field(
        ..., min_length=1, description="Identifiant du document parent."
    )

    content: str = Field(..., description="Contenu textuel du fragment.")

    chunk_index: int = Field(
        ..., ge=0, description="Position du fragment dans le document."
    )
...
class RetrievedDocument(BaseModel):
    ...
    similarity: float = Field(
        ..., ge=0.0, le=1.0, description="Score de similarité vectorielle."
    )

    chunk: DocumentChunk | None = Field(
        default=None, description="Fragment documentaire utilisé."
    )

    excerpt: str | None = Field(
        default=None, description="Extrait court du contenu documentaire."
    )
...
class RetrievalContext(BaseModel):
    ...
    documents: list[RetrievedDocument] = Field(
        default_factory=list, description="Documents sélectionnés par le moteur RAG."
    )

    total_results: int = Field(
        default=0, ge=0, description="Nombre de documents sélectionnés."
    )
