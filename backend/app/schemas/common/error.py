...
class ApiError(BaseModel):
    ...
    code: str = Field(
        ...,
        min_length=1,
        description="Code métier unique de l'erreur."
    )
    ...
    status_code: int = Field(
        ...
        ge=100,
        le=599,
        description="Code HTTP associé à l'erreur."
    )
...
class ValidationIssue(BaseModel):
    ...
    loc: list[str | int] = Field(
        ...
        description="Emplacement du champ invalide."
    )

    msg: str = Field(
        ...,
        min_length=1,
        description="Description du problème de validation."
    )

    type: str = Field(
        ...,
        min_length=1,
        description="Type d'erreur de validation."
    )
...
class ApiValidationError(ApiError):
    ...
    details: list[ValidationIssue] = Field(
        default_factory=list,
        description="Problèmes détectés pendant la validation."
    )
...
class WorkflowWarning(BaseModel):
    ...
    step: WorkflowStep = Field(
        ...,
        description="Étape ayant produit l'avertissement."
    )
    ...
    code: str | None = Field(
        default=None,
        description="Code métier facultatif de l'avertissement."
    )
...
class WorkflowError(BaseModel):
    ...
    step: WorkflowStep = Field(
        ...,
        description="Étape ayant produit l'erreur."
    )
    ...
    recoverable: bool = Field(
        default=True,
        description=(
            "Indique si le workflow peut poursuivre ou produire "
            "un résultat partiel."
        )
    )
