...
REQUIRED_SERVICE_NAMES = frozenset(
    {"mongodb", "milvus", "embedding", "stockfish", "llm", "langgraph"}
)
...
class HealthcheckService:
    ...
    async def _check_service(
        self, *, name: str, ping_function: ServicePingFunction
    ) -> bool:
        ...
        try:
            ...
            logger.exception("Échec du contrôle du service %s.", name)

            return False

    async def _check_services(self) -> ServiceAvailability:
        """Retourne la disponibilité de tous les services."""

        service_checks = {
            "mongodb": ("MongoDB", self._mongodb_service.ping),
            "milvus": ("Milvus", self._milvus_service.ping),
            "embedding": ("Embedding", self._embedding_service.ping),
            "stockfish": ("Stockfish", self._stockfish_service.ping),
            "lichess": ("Lichess", self._lichess_service.ping),
            "youtube": ("YouTube", self._youtube_service.ping),
            "llm": ("LLM", self._llm_service.ping),
            "langgraph": ("LangGraph", self._analysis_service.ping),
        }
        ...
        availabilities = await asyncio.gather(
            *(
                self._check_service(name=display_name, ping_function=ping_function)
                for display_name, ping_function in service_checks.values()
            )
        )

        return dict(zip(service_names, availabilities, strict=True))

    async def _check_required_services(self) -> ServiceAvailability:
    ...
    async def check_readiness(self) -> None:
        ...
        logger.debug("Exécution du contrôle de readiness.")
        ...
        if unavailable_services:
            logger.warning(
                "Application non prête. Services critiques indisponibles : %s.",
                ", ".join(sorted(unavailable_services)),
            )

            raise ApplicationNotReadyError(
                unavailable_services=sorted(unavailable_services)
            )
    ...
    def _get_service_status(self, available: bool) -> ServiceStatus:
    ...
    def _get_application_status(self, services: ServiceAvailability) -> str:
        """Retourne l'état global de l'application."""

        required_services_available = all(
            services.get(service_name, False) for service_name in REQUIRED_SERVICE_NAMES
        )
    ...
    def _build_service_health(self, available: bool) -> ServiceHealth:
        """Construit l'état normalisé d'un service."""

        status = self._get_service_status(available)
        ...
        return ServiceHealth(available=available, status=status, message=message)

    def _build_response(self, services: ServiceAvailability) -> HealthcheckResponse:
        """Construit la réponse complète du healthcheck."""

        return HealthcheckResponse(
            status=self._get_application_status(services),
            ...
            services=ServicesStatus(
                mongodb=self._build_service_health(services["mongodb"]),
                milvus=self._build_service_health(services["milvus"]),
                embedding=self._build_service_health(services["embedding"]),
                stockfish=self._build_service_health(services["stockfish"]),
                lichess=self._build_service_health(services["lichess"]),
                youtube=self._build_service_health(services["youtube"]),
                llm=self._build_service_health(services["llm"]),
                langgraph=self._build_service_health(services["langgraph"]),
            ),
        )
    ...
    async def check(self) -> HealthcheckResponse:
        ...
        logger.debug("Exécution du healthcheck applicatif.")

        services = await self._check_services()

        response = self._build_response(services)

        logger.info("Healthcheck terminé avec le statut %s.", response.status)
    ...
    async def health(self) -> HealthcheckServiceStatus:
