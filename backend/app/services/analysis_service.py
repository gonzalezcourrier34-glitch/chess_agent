...
AnalysisStreamEvent = AnalysisProgressEvent | AnalysisCompletedEvent
...
class AnalysisService:
    ...
    def __init__(
        self, *, graph: ChessAnalysisGraph, dependencies: GraphDependencies
    ) -> None:
    ...
    def _get_graph(self) -> ChessAnalysisGraph:
        """Retourne le graphe compilé."""

        if self._graph is None:
            raise WorkflowConfigurationError(
                context=ErrorContext(
                    workflow="chess_analysis", service="analysis", operation="get_graph"
                ),
                message=("Le workflow LangGraph n'est pas initialisé."),
            )
    ...
    def _normalize_language(self, language: str) -> str:
        ...
        return normalized_language or "fr"

    def _normalize_moves(self, moves: list[str]) -> list[str]:
    ...
    def _build_options(self, request: AnalysisRequest) -> AnalysisOptions:
        ...
        return AnalysisOptions(
            ...
            save_analysis=True,
            response_language=self._normalize_language(request.response_language),
        )

    # Métadonnées

    def _build_metadata(
        self, *, request_id: str, started_at: datetime
    ) -> WorkflowMetadata:
        """Construit les métadonnées initiales du workflow."""

        return WorkflowMetadata(request_id=request_id, started_at=started_at)

    def _complete_metadata(
        self, state: ChessAnalysisState, *, finished_at: datetime, duration_ms: float
    ) -> WorkflowMetadata:
        """Complète les métadonnées après l'exécution."""

        embedding_service = self._dependencies.embedding_service

        embedding_dimension = (
            embedding_service.get_dimension() if embedding_service.is_ready() else None
        )

        stockfish_depth = (
            state.evaluation.engine.evaluation.depth
            if state.evaluation is not None
            else None
        )

        retrieved_document_count = (
            state.retrieval_context.total_results
    ...
    def _build_initial_state(
        self, request: AnalysisRequest, *, request_id: str, started_at: datetime
    ) -> ChessAnalysisState:
        ...
        return ChessAnalysisState(
            ...
            moves=self._normalize_moves(request.moves),
            question=self._normalize_question(request.question),
            options=self._build_options(request),
            status=AnalysisStatus.PENDING,
            metadata=self._build_metadata(request_id=request_id, started_at=started_at),
        )

    # État final

    def _normalize_graph_result(self, result: GraphResult) -> ChessAnalysisState:
        """Convertit le résultat LangGraph en état validé."""

        if isinstance(result, ChessAnalysisState):
            return result

        try:
            return ChessAnalysisState.model_validate(result)

        except Exception as error:
            logger.exception("L'état final retourné par LangGraph est invalide.")
            ...
            raise WorkflowStateError(
                context=ErrorContext(
                    ...
                    metadata={"received_type": type(result).__name__},
                ),
                message=("L'état final du workflow ne respecte pas son schéma."),
                cause=error,
            ) from error
    ...
    def _build_config(self, *, thread_id: str) -> RunnableConfig:
        ...
        config = build_graph_config(self._dependencies)
    ...
    async def _run_graph(
        self, state: ChessAnalysisState, config: RunnableConfig
    ) -> ChessAnalysisState:
        ...
        try:
            ...
            logger.exception("Échec de l'exécution du workflow LangGraph.")
            ...
            raise WorkflowExecutionError(
                ...
                message=("Le workflow d'analyse n'a pas pu terminer son exécution."),
                cause=error,
            ) from error

        return self._normalize_graph_result(result)
    ...
    async def _stream_graph(
        ...
        try:
            ...
            logger.exception("Échec du streaming du workflow LangGraph.")
    ...
    def _extract_step_progress_event(
        self, update: dict[str, Any], *, request_id: str
        ...
        try:
            step = WorkflowStep(current_step)
        ...
        raw_completed_steps = update.get("completed_steps", [])

        completed_steps: list[WorkflowStep] = []

        for completed_step in raw_completed_steps:
            try:
                normalized_step = WorkflowStep(completed_step)

            except ValueError:
                continue

            if normalized_step not in completed_steps:
                completed_steps.append(normalized_step)
    ...
    def _extract_service_progress_event(
        ...
        *,
        ...
        try:
            ...
            service = ServiceType(raw_service) if raw_service is not None else None

        except (KeyError, TypeError, ValueError):
            logger.warning(
                "Événement de progression invalide reçu pendant l'analyse %s.",
                request_id,
            )
    ...
    def _extract_node_updates(self, part: GraphStreamPart) -> list[dict[str, Any]]:
    ...
    def _extract_state_value(self, part: GraphStreamPart) -> ChessAnalysisState | None:
        ...
        logger.debug("État LangGraph reçu via values : type=%s.", type(data).__name__)

        if isinstance(data, ChessAnalysisState):
            return data

        if not isinstance(data, dict):
            logger.warning(
                "État LangGraph ignoré : type inattendu %s.", type(data).__name__
            )

            return None

        try:
            return ChessAnalysisState.model_validate(data)

        except Exception:
            logger.exception("État LangGraph invalide reçu via values.")

            return None

    # Erreurs

    def _extract_error_message(self, state: ChessAnalysisState) -> str | None:
    ...
    def _extract_documents(self, state: ChessAnalysisState) -> list:
    ...
    def _build_response(self, state: ChessAnalysisState) -> AnalysisResponse:
        ...
        return AnalysisResponse(
            ...
            documents=self._extract_documents(state),
            videos=list(state.videos),
            explanation=state.response,
            analysis_id=state.analysis_id,
            error=self._extract_error_message(state),
        )
    ...
    async def analyze(
        ...
        effective_request_id = request_id if request_id is not None else str(uuid4())

        started_at = datetime.now(UTC)

        started_counter = perf_counter()

        logger.info("Démarrage de l'analyse %s.", effective_request_id)
        ...
        initial_state = self._build_initial_state(
            request, request_id=effective_request_id, started_at=started_at
        )

        # Configuration

        config = self._build_config(thread_id=effective_request_id)
        ...
        duration_ms = round((perf_counter() - started_counter) * 1_000, 2)

        final_state = final_state.model_copy(
            update={
                "metadata": self._complete_metadata(
                    final_state, finished_at=finished_at, duration_ms=duration_ms
                )
            }
        )
        ...
        logger.debug(
            ...
            (
                len(final_state.retrieval_context.documents)
                if final_state.retrieval_context is not None
                else 0
            ),
            len(final_state.videos),
            len(final_state.warnings),
        ...
        logger.info(
            "Analyse %s terminée en %.2f ms avec le statut %s.",
            effective_request_id,
            duration_ms,
            final_state.status.value,
        )
    ...
    async def stream_analysis(
        ...
        logger.info("Démarrage du streaming de l'analyse %s.", effective_request_id)
        ...
        async for part in self._stream_graph(initial_state, config):
            part_type = part.get("type")

            # État complet

            if part_type == "values":
                state_value = self._extract_state_value(part)
            ...
            if part_type == "custom":
                ...
                event = self._extract_service_progress_event(
                    data,
                ...
                logger.debug(
                    ("Analyse %s : service=%s step=%s status=%s."),
                    effective_request_id,
                    (event.service.value if event.service is not None else None),
                    event.step.value,
                    event.status.value,
                )
            ...
            for update in self._extract_node_updates(part):
                event = self._extract_step_progress_event(
                    update, request_id=effective_request_id
                )

                if event is None:
                    continue

                completed_steps = list(event.completed_steps)

                logger.debug(
                    ("Analyse %s : étape=%s status=%s."),
                    effective_request_id,
                    event.step.value,
                    event.status.value,
                )
        ...
        if latest_state is None:
            raise WorkflowStateError(
                ...
                message=("Le streaming s'est terminé sans état final exploitable."),
            )
        ...
        final_state = latest_state.model_copy(
            update={
                "metadata": self._complete_metadata(
                    latest_state, finished_at=finished_at, duration_ms=duration_ms
                )
            }
        )
        ...
        logger.info(
            "Streaming de l'analyse %s terminé en %.2f ms avec le statut %s.",
            effective_request_id,
            duration_ms,
            final_state.status.value,
        )
        ...
        yield AnalysisCompletedEvent(
            request_id=effective_request_id, analysis=self._build_response(final_state)
        )
