...
class ChessService:
    ...
    def build_position(
        ...
        return BoardPosition(
            fen=board.fen(),
            active_color=self._get_board_color(
                board
            ),
    ...
    def get_active_color(
        ...
        return self._get_board_color(
            board
        )

    def _get_board_color(
        self,
        board: chess.Board
    ) -> ChessColor:
    ...
    def get_legal_moves(
        ...
        return self._build_legal_moves(
            board
        )
    ...
    def _ensure_legal_move(
        ...
        if move not in board.legal_moves:
            raise InvalidMoveError(
                message=(
                    "Le coup fourni n'est pas légal "
                    "dans cette position."
                )
            )
    ...
    def parse_move(
        ...
        try:
            return self.parse_uci_move(
                board,
                normalized_move
            )

        except InvalidMoveError:
            return self.parse_san_move(
                board,
                normalized_move
            )
    ...
    def parse_san_move(
        ...
        try:
            ...
            raise InvalidMoveError(
                message=(
                    "Le coup fourni n'est pas une notation "
                    "SAN valide ou légale."
                )
            ) from error
    ...
    def convert_move(
        ...
        source_notation = normalize_notation(
            source,
            required=True
        )

        target_notation = normalize_notation(
            target,
            required=True
        )

        if source_notation is MoveNotation.UCI:
            parsed_move = self.parse_uci_move(
                board,
                move
            )

        else:
            parsed_move = self.parse_san_move(
                board,
                move
            )
    ...
    def convert_uci_history_to_san(
        self,
        moves: list[str]
    ) -> list[str]:
    ...
    def apply_move(
        ...
        parsed_move = self.parse_move(
            board,
            move
        )

        legal_move = self.build_move(
            board,
            parsed_move
        )
